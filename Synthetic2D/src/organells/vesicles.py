...
class Vesicles(Location):
            ...
            while True:
                ...
                if not self.CheckOverlap(self.Points, now_point, nowSizeCycle, 0.8):
                    ...
                    if np.random.random() < 0.05:
                        ########################################################################### НУЖНО ПРОВЕРИТЬ ЛОГИКУ РАБОТЫ
                        size_cycle_x = max(np.random.randint(nowSizeCycle-1, nowSizeCycle+1), 3)
                        size_cycle_y = max(np.random.randint(nowSizeCycle-1, nowSizeCycle+1), 3)
                        self.listSizeVesiculs.append((size_cycle_x, size_cycle_y))
                    else:
    ...
    def Draw(self, image, use_rand_color = False, minus_radius_val = 0):    #minus_radius_val для коррекции разметки, так как из-за размытия она страдает
        ...
        for i in range(self.numberPoints):
            ...
            radius_ves = self.listSizeVesiculs[i]
