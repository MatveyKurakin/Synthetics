...
with open('AC4.json', 'r', encoding='utf-8') as f:
    PARAM = json.load(f)
...
'''
# для того чтобы создать json файлик нужно раскоментировать нужный PARAM и сохранение (пока что не посмотрел можно ли делать коментарии в json файле
